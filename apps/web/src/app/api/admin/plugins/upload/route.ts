...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const tempZipPath = path.join(tempDir, `${Date.now()}_plugin.zip`);
    ...
    try {
      ...
      if (!fs.existsSync(manifestPath)) {
        return apiError('BAD_REQUEST', 'Missing manifest.json inside plugin package', null, 400);
      }
      ...
      const finalDest = path.join(root, 'packages', 'plugins', 'custom', manifest.slug);
      ...
      return apiSuccess({ slug: manifest.slug, name: manifest.name }, 'Plugin uploaded and extracted successfully');
    } finally {
      // Clean up temp zip
    ...
    return apiError('UPLOAD_ERROR', error.message || 'Error occurred uploading plugin', null, 500);
  }
}
